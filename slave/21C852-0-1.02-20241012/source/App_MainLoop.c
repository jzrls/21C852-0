...
#include "F28335_Device.h"     /* F28335 Õ‚…ËÕ∑Œƒº˛*/
#include "App_UserDef.h"      /* F28335 ”¶”√œÓƒøÕ∑Œƒº˛*/
...
//	SPI∂¡–¥≤Ÿ◊˜
	...
	RDC_DIS_A;
	for(i = 0;i < 128;i++)
		READ_X25040(&SCI_RX_BUFFER[0+i],128+i,1,0);	// ∂‘”¶…œŒªª˙øÿ÷∆≤Œ ˝ΩÁ√Ê
	...
	Udc_Setg= SCI_ReceData[6];      //’‚¿Ô «…œŒªª˙…Ë÷√µƒ÷±¡˜µÁ—πµƒµπ ˝
	Udc_Mche_realvalue = _IQdiv(_IQ(1),Udc_Setg);  //’‚¿Ô «…œŒªª˙…Ë÷√µƒ÷±¡˜µÁ—πµƒ÷µ

	U_Base = SCI_ReceData[7];
	I_Base = SCI_ReceData[8];
	V_Base = SCI_ReceData[9];		// µÁÀŸ∂»ª˘÷µ(rad/s)
	...
	for(i = 0;i < 128;i++)
		READ_X25040(&SCI_RX_BUFFER[0+i],0+i,1,0);	// ∂‘”¶…œŒªª˙øÿ÷∆≤Œ ˝ΩÁ√Ê
	Sci_DataCombine_Rx(SCI_RX_BUFFER);
	...
	for(i = 0;i < 128;i++)
		READ_X25040(&SCI_RX_BUFFER[0+i],0+i,1,8);	// ∂‘”¶…œŒªª˙øÿ÷∆≤Œ ˝ΩÁ√Ê
	...
	for(i = 0;i < 128;i++)
		READ_X25040(&SCI_RX_BUFFER[0+i],128+i,1,8);	// ∂‘”¶…œŒªª˙øÿ÷∆≤Œ ˝ΩÁ√Ê
...
//-------------------------------------------------------------------------------
	...
	ScicRegs.SCIFFTX.bit.TXFIFOXRESET=1;
   	...
   	ECanaRegs.CANGIF0.all=0xFFFFFFFF;       // «ÂÀ˘”–÷–∂œ±Í÷æŒª
	...
	EALLOW;
	EPwm1Regs.ETCLR.bit.INT = 1;             //«Â÷–∂œ±Í÷æ
	...
	for(;;)
	{
		...
		if(SCIB.FLAG.bit.RX_FLAG == 1)
		{
			if(SCI_RX_BUFFER[2] == 0xA0)
			{
		 		 ...
		 		 || ((BORAD_NUM == 1 || BORAD_NUM == 3 || BORAD_NUM == 5) && BC_SLT_IN == 1) )
				{
					...
					if(SCIB.FLAG.bit.TXBO_FLAG ==1)
					{
						...
						*SCIB.p_Tx_Buffer++ = (Data_Temp & 0x00FF); 					/* ¿€º”«Û–ß—È∫Õ*/
						*SCIB.p_Tx_Buffer = (Data_Temp>>8); 					/* ¿€º”«Û–ß—È∫Õ*/
						SCIB.p_Tx_Buffer = SCI_TX_BUFFER;	
						SCIB.m_Tx_Length = 8+7;

						SCIB.FLAG.bit.TXBO_FLAG = 0;
				   		//ScicRegs.SCITXBUF = *SCIB.p_Tx_Buffer++;
						//ScicRegs.SCIFFTX.bit.TXFFIENA = 1;// πƒ‹∑¢ÀÕ÷–∂œ
					}
				}
			}
			...
			{	
				...
				CAN_BUFFER_RX[1] = 	0x07;//œ»—°‘Ò¥”∞Â
				//»Áπ˚÷˜∞ÂŒﬁπ ’œ£¨«“¥”∞Â≤ª‘⁄‘À––÷–£¨‘Ú—°‘Ò÷˜∞Â‘À––
				if((CAN_BUFFER_RT[0][2] & 0x02) == 0x00 && (CAN_BUFFER_RT[1][2] & 0x01) == 0x01)
				...
				for(i = 2; i < 7; i++)
					...
					else if(CAN_BUFFER_RX[2] == 0x14)
					{
						 ...
						 * –ﬁ∏ƒ»À£∫¿Ó ¢
						 * –ﬁ∏ƒ¿‡–Õ£∫–¬‘ˆ
						 * –ﬁ∏ƒƒ⁄»›£∫ π”√±‰¡øsetSpeedº«¬º…œŒªª˙œ¬∑¢µƒ◊™ÀŸ
						 **/
		 		 ...
		 		 || ((BORAD_NUM == 1 || BORAD_NUM == 3 || BORAD_NUM == 5) && BC_SLT_IN == 1) )
				{
					...
					if(SCIB.FLAG.bit.TXBO_FLAG ==1)
					{
						...
						*SCIB.p_Tx_Buffer++ = (Data_Temp & 0x00FF); 					/* ¿€º”«Û–ß—ÈÕ*/
						*SCIB.p_Tx_Buffer = (Data_Temp>>8); 					/* ¿€º”«Û–ß—È∫Õ*/
						SCIB.p_Tx_Buffer = SCI_TX_BUFFER;	
			...
			SCIB.FLAG.bit.RX_FLAG = 0;		    // Ω” ’√¸¡Ó¥¶¿ÌÕÍ±œ”¶¥
		}
	    ...
	    if(CANB.FLAG.bit.RX_FLAG == 1)
		{
			...
			if(Board_Num == BORAD_NUM)
				Can_Deal();
			CANB.FLAG.bit.RX_FLAG = 0;		    // Ω” ’√¸¡Ó¥¶¿ÌÕÍ±œ”¶¥
		}
	    ...
	    if(CANB2.FLAG.bit.RX_FLAG == 1)
		{
			...
			if(Board2_Num == BORAD_NUM)
				Can_Receive();
			CANB2.FLAG.bit.RX_FLAG = 0;		    // Ω” ’√¸¡Ó¥¶¿ÌÕÍ±œ”¶¥
		}
