...
// ÎÄ¼þÃû:	 App_Main.h
//
// ¹¦ÄÜÃèÊö: Ö÷³ÌÐò±äÁ¿¼´ËùÓÐÈ«¾Ö±äÁ¿(½á¹¹Ìå)¶¨Òå¡£
//			 ¶¨ÒåÍêºóÐëÔÚApp_UserDef.h ÖÐÉêÃ÷ÎªÍâ²¿±äÁ¿,ÒÔ±ãËùÓÐÎÄ¼þ¾ù¿É·ÃÎÊ¡£
//
// ±àÐ´Õß:   ÁõÈÐ		
// ±àÐ´ÈÕÆÚ: 2021-05-10
//
...
_iq	SCI_ReceData[30];		//´®¿Ú½ÓÊÕµÄÊý¾Ý
_iq	SCI_SendData[30];		//´®¿Ú·¢ËÍµÄÊý¾Ý
...
//======================================================================================================
	...
	Uint16	EEPROM_BUFFER[RX_BUFFER_LENGTH] = {0};	// EEPROMÊý¾Ý»º³åÇø
	...
	_iq	ReceData[30];		//´®¿Ú½ÓÊÕµÄÊý¾Ý
	_iq	SendData[30];		//´®¿Ú·¢ËÍµÄÊý¾Ý
	...
	volatile 	_iq    	Vel_Speed = _IQ(0);			//¸ßËÙÄ£Ê½×ªËÙ
...
//======================================================================================================
//È«¾Ö±äÁ¿»ò½á¹¹ÌåµÄ¶¨Òå
	_iq30 *pArcSin = (_iq30 *)0x00318000;		// Ö¸Ïò·´ÕýÏÒº¯Êý±í£¬Q30
...
//¶¨ÒåPWMAºÍPWMBÇý¶¯Æ÷
	...
	volatile Uint16		Control_Period = CARRY_A_P;	// Q16 ¿ØÖÆÖÜÆÚ¼ÆÊýÖµ
	volatile Uint16		Deadtime = PWM_DeadTime;	// Q16 ËÀÇø¼ÆÊýÖµ
	volatile _iq		Velo_Duty = _IQ(1000);				// QG ×ªËÙÔØ²¨ÏµÊý
	volatile _iq		Velo_Duty0 = _IQ(1000);				// QG ×ªËÙÔØ²¨ÏµÊý
 	volatile _iq		K_Velo_Cal = K_VELO_CALC;	// ËÙ¶È¼ÆËã²ÎÊý
...
//======================================================================================================
	volatile Uint16	 	Run_Led_Index = 0;				// ÔËÐÐµÆÔËÐÐµÄ¿ØÖÆÖÜÆÚµÄÖ¸Êý
...
//------------------------------------------------------
// ÆµÂÊ¡¢ËÙ¶ÈÒÔ¼°½Ç¶È
	volatile Uint16		Angle_Q16 = 0;					// Q16 Ðý±ä²âÈ¡µÄµ±Ç°Î»ÖÃ½ÇHEX
	volatile Uint16		Angle0_Q16 = 0;					// Q16 Ðý±ä²âÈ¡µÄÉÏÒ»´ÎÎ»ÖÃ½ÇHEX
	volatile Uint16	    Angle_Init_Digital = INIT_ANGLE;	// Q0 ÓÃÓÚÐý±ä²âÁ¿µÄ×ª×Ó³õÎ»ÖÃ½Ç(HEX)
	volatile Uint16	    Angle_Init_Fa = 0;	// Q0 ÓÃÓÚÐý±ä²âÁ¿µÄ×ª×Ó³õÎ»ÖÃ½Ç(HEX)
	volatile Uint16		Angle_Fa_Q16 = 0;					// Q16 Ðý±ä²âÈ¡µÄµ±Ç°Î»ÖÃ½ÇHEX
	...
	volatile _iq setSpeed = _IQ(0); /* ÀîÊ¢ 20241007ÐÂÔö */
	volatile _iq prevSpeed = _IQ(0);

	volatile _iq		Pos_Set=0;                  	 // QG ¸ø¶¨µÄÎ»ÖÃ
	
	volatile 	long	Velo_Calc_Num = 10;     //ËÙ¶ÈÆ½¾ù´ÎÊý
	volatile 	Uint16	Velo_Avg_Index = 0;     //ËÙ¶ÈÆ½¾ùÖ¸Êý
	volatile	_iq		Velotmp = _IQ(0);
	volatile	Uint16	Velo_Calc_flag = 0;
//----------------------------------------------------------
// ÓÃÓÚ´æ´¢Ö±½ÓADC²ÉÑùµÄµ½Êý×ÖÁ¿,ÐèÒª½øÐÐÐ£Õý
...
//------------------------------------------------------------
	...
	volatile 	_iq		Us = 0;					        // QG Us=mag(ud,uq)µÄ±êçÛÖµ
 	...
 	volatile 	_iq    	Udc_Mche = _IQ(1);			// QG Ö±Á÷²àµçÑ¹µÄ²âÁ¿Öµ,ÉÏÎ»»ú¸ø¹ýÀ´µÄÊÇÖ±Á÷²àµçÑ¹¸ø¶¨ÖµµÄµ¹Êý
	...
	volatile 	_iq    	Isd_Set = _IQ(0);      //ÉÏÎ»»ú·¢ËÍ¹ýÀ´µÄid
	...
	volatile 	_iq		Us_Max = 0;					// QG Ud£¬UqµÄÏÞ·ù k*0.612Udc (ºã¹¦ÂÊ±ä»»)
	...
	//-------------------------------------------------------------
 	...
 	// µç»ú²ÎÊý
	volatile    _iq  		Ld = K_LD;			       // µç»údÖáµç¸ÐµÄ±êçÛÖµ
	volatile    _iq  		Lq = K_LQ;			       // µç»úqÖáµç¸ÐµÄ±êçÛÖµ
	volatile    _iq  		PHI = K_PHI;	           // µç»ú´ÅÁ´µÄ±êçÛÖµ
	volatile    _iq  		Rs = K_RS;			       // µç»úµç×èµÄ±êçÛÖµ
	volatile    _iq  		Mp = K_MP;			       // µç»úµÄ¼«¶ÔÊý
	volatile    _iq  		Rp = K_RP;			       // µç»úµÄ¼«¶ÔÊý
	volatile    _iq  		p = K_P;			       // µç»úÓëÐý±äµÄ¼«¶ÔÊý±È
	// »ùÖµÏµÍ³
...
//====================================================================================
	...
	volatile	_iq 	Temperature1 = 0;		// ÎÂ¶ÈÐÅºÅ1
	volatile	_iq 	Temperature3 = 0;		// ÎÂ¶ÈÐÅºÅ3
	volatile	_iq 	Temperature5 = 0;		// ÎÂ¶ÈÐÅºÅ4
	volatile	_iq 	Temperature10 = 0;		// ÎÂ¶ÈÅºÅ1
	volatile	_iq 	Temperature30 = 0;		// ÎÂ¶ÈÐÅºÅ3
	volatile	_iq 	Temperature50 = 0;		// ÎÂ¶ÈÐÅºÅ4
	volatile	_iq 	Temperature1_Off = 0;	// ÎÂ¶ÈÐÅºÅ1
	volatile	_iq 	Temperature3_Off = 0;	// ÎÂ¶ÈÐÅºÅ3
	volatile	_iq 	Temperature5_Off = 0;	// ÎÂ¶ÈÐÅºÅ4
	volatile	_iq 	Temperature1_K = 0;		// ÎÂ¶ÈÐÅºÅ1
	volatile	_iq 	Temperature3_K = 0;		// ÎÂ¶ÈÐÅºÅ3
	volatile	_iq 	Temperature5_K = 0;		// ÎÂ¶ÈÐÅºÅ4
