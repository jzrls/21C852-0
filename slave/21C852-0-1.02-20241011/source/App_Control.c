...
void Ready_DataA(void)
{
	...
	// 计算速度
		...
		if( Velo_Avg_Index >= Velo_Calc_Num )	// 判断是否需要计算速度，Velo_Calc_Num不是Q20的数！！！
		{
			...
			if(Sys_Flag.bit.STOP_PWM_Flag_Velo == 1 && Velo_Elec_abs < Velo_Max
...
//			|| (_IQmag(IdZ_Set, IqZ_Set) >= (idSet_Max>>1) && Is <= (idSet_Max>>3)) )
	...
	if(Ctrl_Flag.bit.UdcLow_Flag == 1 && _IQabs(Udc_Mche_1) > UdcLimit1_Set
			&& Velo_Elec_abs < Velo_Start)       //直流侧电压下限
	{
		UdcLowEnable++;
...
//	if(Temperature1 >= MotorTM || Temperature3 >= MotorTM)       //电机过温
//	{
//		Temp1OverProtect++;
...
void ControlA(void)
{
	...
	if(Ctrl_Flag.bit.CANERROR_FLAG == 0 && Sys_Flag.bit.STOP_PWM_Flag_Velo == 0
		&& Ctrl_Flag.bit.speedFluctuation == 0 && Ctrl_Flag.bit.Temp2Over_Flag == 0
		&& Sys_Flag.bit.STOP_PWM_Flag_Id == 0 && Sys_Flag.bit.UDC_FLAG == 0
		&& Sys_Flag.bit.STOP_PWM_Flag_Driv == 0 && Ctrl_Flag.bit.UdcLow_Flag == 0 
		&& Fault_Flag.bit.RDC1_FAULT == 0 && Fault_Flag.bit.RDC2_FAULT == 0)
	...
	{
		if(Ctrl_Flag.bit.LOCK_FLAG == 1 && Sys_Flag.bit.STOP_PWM_Flag_Velo == 0 && Ctrl_Flag.bit.speedFluctuation == 0
			&& Sys_Flag.bit.STOP_PWM_Flag_Id == 0 && Sys_Flag.bit.STOP_PWM_Flag_Driv == 0)
		{
			PWMA_Update();
			MeasureInitDigital();
		}
		else if(Ctrl_Flag.bit.OPEN_LOOP_FLAG == 1 && Sys_Flag.bit.STOP_PWM_Flag_Velo == 0 && Ctrl_Flag.bit.speedFluctuation == 0
			&& Sys_Flag.bit.STOP_PWM_Flag_Id == 0 && Sys_Flag.bit.STOP_PWM_Flag_Driv == 0)
		{
			PWMA_Update();
			OpenLoop();
