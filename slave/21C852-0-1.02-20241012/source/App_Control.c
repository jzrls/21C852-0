...
void Ready_DataA(void)
{
	...
	static Uint16 usec = 0; // Î¢Ãî¼ÆÊýÆ÷£¬150us×ÔÔö1
	static Uint16 msec = 0; // ºÁÃë¼ÆÊýÆ÷
	static Uint16 sec = 0; // Ãë¼ÆÊýÆ÷
    static Uint16 startTime = 6; // Æô¶¯½×¶Î²»½øÐÐ×ªËÙ²¨¶¯ÅÐ¶Ï£¨6sÄÚ²»ÅÐ¶Ï£©£¬ÏÖÊµ¼ÊÆô¶¯Ê±¼ä1s
    ...
    static Uint16 speedMaxProtectionCount = 2000; // Á¬Ðø¼ì²â3s£¨3000000us/150us*10% = 2000£©£¬´æÔÚ10%×ªËÙ²¨¶¯½øÐÐÇÐ°å²Ù×÷ 
...
//	ËÙ¶ÈÂË²¨
	...
	/**
	 * Ö÷°å¼ì²âµ½×ªËÙ²¨¶¯ºó½øÐÐÇÐ°å²Ù×÷
	 * ²¨¶¯·¶Î§£ºÉè¶¨×ªËÙµÄ20%
	 * ²¨¶¯´ÎÊý£º¼ì²â´ÎÊýµÄ10%
	 * Ö÷°åÇÐ»»£¬¸±°å×ªËÙ²¨¶¯Ê±²»¼ì²â£¬²¨¶¯¹ý´óÈËÎªÍ£»ú
	 **/
	if (BORAD_NUM == 2) {
		// Éè¶¨×ªËÙÎª0Ê±£¬¼ÆÊýÆ÷Çå0£¬×ªËÙÇÐ»»Ê±Ò²ÖØÐÂ¼ÆÊ±
		...
		/* ¸ù¾ÝÉè¶¨×ªËÙ¼ÆËã×ªËÙ²¨¶¯ãÐÖµ£ºÉè¶¨×ªËÙ*×ªËÙ²¨¶¯±ÈÀý£¨20%£© */
		...
		/*
		 * Æô¶¯½áÊø»ò×ªËÙÇÐ»»Íê³ÉÖ®ºó½øÐÐ×ªËÙ²¨¶¯ÅÐ¶Ï
		 **/
		if (sec >= startTime) {		
			/* ºÁÃë¼ÆÊ± */
			protectionUsec++;
