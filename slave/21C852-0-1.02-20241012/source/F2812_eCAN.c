...
void Can_Rt_Data(Uint16 Num)
{
	...
	if(Num == 1)
	{
		...
		{
			//½«×ªËÙ±êçÛÖµ×ª»»ÎªIQ15µÄÊµ¼Ê×ªËÙÖµ£¬ÔÙ×ª»»Îª16Î»ÕûÊý,µÍ×Ö½ÚÔÚÇ°
		...
		{
			//½«16Î»µÄÎ»ÖÃ×ª»»Îª½Ç¶ÈÖµ£¬ÔÙ*100×ª»»Îª16Î»ÕûÊý,µÍ×Ö½ÚÔÚÇ°
			tmp = Angle_Fa_Q16;
		...
		//ÎÂ¶ÈÖµ×ª»»ÎªIQ15µÄÊµ¼ÊÖµ£¬ÔÙ½«Æä×ª»»Îªµ¥¾«¶È¸¡µãÊý¶ÔÓ¦µÄÊ®Áù½øÖÆ±íÊ¾
		...
		//½«Ä¸ÏßµçÑ¹±êçÛÖµ×ª»»ÎªIQ15µÄÊµ¼ÊÖµ£¬ÔÙ½«Æä×ª»»Îªµ¥¾«¶È¸¡µãÊý¶ÔÓ¦µÄÊ®Áù½øÖÆ±íÊ¾
		if(Udc_Mche_1 <= _IQ(0))
			tmp = 0;
		else
		{
	...
	if(Num == 2)
	{
		//½«Ä¸ÏßµçÁ÷±êçÛÖµ×ª»»ÎªIQ15µÄÊµ¼ÊÖµ£¬ÔÙ½«Æä×ª»»Îªµ¥¾«¶È¸¡µãÊý¶ÔÓ¦µÄÊ®Áù½øÖÆ±íÊ¾
		...
		//½«AÏàµçÁ÷±êçÛÖµ×ª»»ÎªIQ15µÄÊµ¼ÊÖµ£¬ÔÙ½«Æä×ª»»Îªµ¥¾«¶È¸¡µãÊý¶ÔÓ¦µÄÊ®Áù½øÖÆ±íÊ¾
		...
		//½«BÏàµçÁ÷±êçÛÖµ×ª»»ÎªIQ15µÄÊµ¼ÊÖµ£¬ÔÙ½«Æä×ª»»Îªµ¥¾«¶È¸¡µãÊý¶ÔÓ¦µÄÊ®Áù½øÖÆ±íÊ¾
		...
		//½«CÏàµçÁ÷±êçÛÖµ×ª»»ÎªIQ15µÄÊµ¼ÊÖµ£¬ÔÙ½«Æä×ª»»Îªµ¥¾«¶È¸¡µãÊý¶ÔÓ¦µÄÊ®Áù½øÖÆ±íÊ¾
...
interrupt void ECana_T(void)
{
	...
	if(ECanaRegs.CANRMP.bit.RMP0==1)   /*µÈ´ýËùÓÐRMP0ÖÃÎ»*/
  	{
  		if(CANB.FLAG.bit.RX_FLAG == 0)// && CANB.FLAG.bit.TXBO_FLAG == 1)
	   {
			...
			*CANB.p_Rx_Buffer = ECanaMboxes.MBOX0.MDH.byte.BYTE5;
			...
			//¼ì²é°üÍ·ÊÇ·ñÎªAA55,ÒÔ¼°»ñÈ¡Êý¾Ý³¤¶È----------------------
			...
			else
			{
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP0 =1;     /*Çå³ýËùÓÐRMP0Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP3==1)   /*µÈ´ýËùÓÐRMP0ÖÃÎ»*/
  	{
  		if(CANB2.FLAG.bit.RX_FLAG == 0)// && CANB2.FLAG.bit.TXBO_FLAG == 1)
	   {
			...
			*CANB2.p_Rx_Buffer = ECanaMboxes.MBOX3.MDH.byte.BYTE5;
			...
			else
			{
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP3 =1;     /*Çå³ýËùÓÐRMP0Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP5==1)   /*µÈ´ýËùÓÐRMP5ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 0)
		{
			...
			CAN_BUFFER_RT[0][8]= 0;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP5 =1;     /*Çå³ýËùÓÐRMP5Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP6==1)   /*µÈ´ýËùÓÐRMP6ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 1)
		{
			...
			CAN_BUFFER_RT[1][8]= 0;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP6 =1;     /*Çå³ýËùÓÐRMP6Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP7==1)   /*µÈ´ýËùÓÐRMP7ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 2)
		{
			...
			CAN_BUFFER_RT[2][8]= 0;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP7 =1;     /*Çå³ýËùÓÐRMP7Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP8==1)   /*µÈ´ýËùÓÐRMP8ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 3)
		{
			...
			CAN_BUFFER_RT[3][8]= 0;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP8 =1;     /*Çå³ýËùÓÐRMP8Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP9==1)   /*µÈ´ýËùÓÐRMP9ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 4)
		{
			...
			CAN_BUFFER_RT[4][8]= 0;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP9 =1;     /*Çå³ýËùÓÐRMP9Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP10==1)   /*µÈ´ýËùÓÐRMP10ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 5)
		{
			...
			CAN_BUFFER_RT[5][8]= 0;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP10 =1;     /*Çå³ýËùÓÐRMP10Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 

	if(ECanaRegs.CANRMP.bit.RMP12==1)   /*µÈ´ýËùÓÐRMP10ÖÃÎ»*/
  	{
		...
		CAN_BUFFER_CTRL_Num++;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP12 =1;     /*Çå³ýËùÓÐRMP10Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 

	if(ECanaRegs.CANRMP.bit.RMP14==1)   /*µÈ´ýËùÓÐRMP14ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 0)
		{
			...
			CAN_BUFFER_RT[0][16] = ECanaMboxes.MBOX14.MDH.byte.BYTE5;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP14 =1;     /*Çå³ýËùÓÐRMP14Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP15==1)   /*µÈ´ýËùÓÐRMP15ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 1)
		{
			...
			CAN_BUFFER_RT[1][16] = ECanaMboxes.MBOX15.MDH.byte.BYTE5;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP15 =1;     /*Çå³ýËùÓÐRMP15Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP16==1)   /*µÈ´ýËùÓÐRMP16ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 2)
		{
			...
			CAN_BUFFER_RT[2][16] = ECanaMboxes.MBOX16.MDH.byte.BYTE5;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP16 =1;     /*Çå³ýËùÓÐRMP16Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP17==1)   /*µÈ´ýËùÓÐRMP17ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 3)
		{
			...
			CAN_BUFFER_RT[3][16] = ECanaMboxes.MBOX17.MDH.byte.BYTE5;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP17 =1;     /*Çå³ýËùÓÐRMP17Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP18==1)   /*µÈ´ýËùÓÐRMP18ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 4)
		{
			...
			CAN_BUFFER_RT[4][16] = ECanaMboxes.MBOX18.MDH.byte.BYTE5;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP18 =1;     /*Çå³ýËùÓÐRMP18Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	} 
	if(ECanaRegs.CANRMP.bit.RMP19==1)   /*µÈ´ýËùÓÐRMP19ÖÃÎ»*/
  	{
  		if(BORAD_NUM != 5)
		{
			...
			CAN_BUFFER_RT[5][16] = ECanaMboxes.MBOX19.MDH.byte.BYTE5;
  	   	...
  	   	ECanaRegs.CANRMP.bit.RMP19 =1;     /*Çå³ýËùÓÐRMP19Î»²¢¿ªÊ¼½ÓÊÕÏûÏ¢*/
	}
...
//----------------------------------------------------------------------------------------
// ·¢ËÍÊý¾Ý´ò°ü³ÌÐò
// ½«ÐèÒª·¢ËÍµÄÊý¾Ý°´ÕÕÍ¨Ñ¶¹æÔ¼´ò°ü³ÉÊýÝèå
...
void CAN_DataPackage_Tx(_iq	*p_tx_data_L, Uint16 m_tx_command_L, Uint16 m_tx_length_L)
{
	...
	*CANB.p_Tx_Buffer++ = (Data_Temp & 0x00FF); 					/* ÀÛ¼ÓÇóÐ§ÑéºÍ*/
	*CANB.p_Tx_Buffer = (Data_Temp>>8); 					/* ÀÛ¼ÓóÐ§ÑéºÍ*/
...
void CAN2_DataPackage_Tx(_iq	*p_tx_data_L, Uint16 m_tx_command_L, Uint16 m_tx_length_L)
{
	...
	*CANB2.p_Tx_Buffer++ = (Data_Temp & 0x00FF); 					/* ÀÛ¼ÓÇóÐ§ÑéºÍ*/
	*CANB2.p_Tx_Buffer = (Data_Temp>>8); 					/* ÀÛ¼ÓÇóÐ§ÑéºÍ*/
	...
	switch(command)
   	...
   	case 0x0000:		//³õÊ¼»¯¼ì²â¿ØÖÆÆ÷Ö¸Áî			lxy
		...
		SendData[1] = BORAD_NUM;
   		...
   		break;
	//---------------------------------------------------------------------------
	case 0x0001:		//Çå³ý¹ÊÕÏ±êÖ¾Ö¸Áî			lxy
			...
			Ctrl_Flag.bit.speedFluctuation = 0; /* 20241013 ÀîÊ¢ ÐÂÔö */
   	...
   	//------------------------------------------------------------
   		...
   		case 0x0004:		//Ð´µç»ú²ÎÊýºÍ»ùÖµÏµÍ³Ö¸Áî			lxy
   			...
   			p = _IQdiv(ReceData[4],ReceData[5]);
   	   		...
   	   		U_Base = ReceData[7];
   	   		I_Base = ReceData[8];
   	   		V_Base = ReceData[9];		// µçËÙ¶È»ùÖµ(rad/s)
			...
			pi_Iq_Kc.Ki_reg		= pi_Id_Kc.Ki_reg;
   	   	...
   	   	//---------------------------------------------------------------------------
   	    	...
   	    	//---------------------------------------------------------------------------
   	    	case 0x0005:		//¶Áµç»ú²ÎÊýºÍ»ùÖµÏµÍ³Ö¸Áî		lxy
   	    		SendData[0] = Rs;
   	    		SendData[1] = Ld;
   	 	 	    SendData[2] = Lq;
   	    		SendData[3] = PHI;
   	    		...
   	    		SendData[7] = U_Base;
   	    		SendData[8] = I_Base;
   	    		SendData[9] = V_Base;			// µçÐµËÙ¶È»ùÖµ(rad/s)
...
//---------------------------------------------------------------------------
   	 	...
   	 	case 0x0009:		//¶ÁÈëµç»ú´«¸ÐÆ÷Ö¸Áî			lxy
				CAN_DataCombine_Rx(RX_BUFFER);
				Choose_Mche= ReceData[0];
   		...
   		case 0x000C:		//¶Á²ÎÊýÖ¸Áî		lxy
   	   		...
   	   		break;
   	   	   	//---------------------------------------------------------------------------
   	   	//---------------------------------------------------------------------------
   	   	case 0x0010:		//Ð´Ä£âÍ¨µÀÊýÖ¸Áî		lr
			...
			//Sys_Flag.bit.RotorWay_Flag = ReceData[13];
   	 		...
   	 		Velo_Start = ReceData[15];	// Æô¶¯ËÙ¶È
   	 		Vel_Speed = ReceData[16];
   	 		Ctrl_Flag.bit.VelWay_Flag = ReceData[17];
   	  		TorquetoIs = ReceData[18];	// ×ª¾ØµçÁ÷±È
