...
//---------------------------------------------------------
	...
	if(Ctrl_Flag.bit.UdcLow_Flag == 1 && _IQabs(Udc_Mche_1) > UdcLimit1_Set
			&& Velo_Elec_abs < Velo_Start)       //Ö±Á÷²àµçÑ¹ÏÂÏÞ
	{
		UdcLowEnable++;
...
void ControlA(void)
{
	...
	if(Ctrl_Flag.bit.CANERROR_FLAG == 0 && Sys_Flag.bit.STOP_PWM_Flag_Velo == 0
		&& Ctrl_Flag.bit.Temp1Over_Flag == 0 && Ctrl_Flag.bit.Temp2Over_Flag == 0 
	...
	{
		if(Ctrl_Flag.bit.LOCK_FLAG == 1 && Sys_Flag.bit.STOP_PWM_Flag_Velo == 0
			&& Sys_Flag.bit.STOP_PWM_Flag_Id == 0 && Sys_Flag.bit.STOP_PWM_Flag_Driv == 0)
		{
			PWMA_Update();
			MeasureInitDigital();
		}
		else if(Ctrl_Flag.bit.OPEN_LOOP_FLAG == 1 && Sys_Flag.bit.STOP_PWM_Flag_Velo == 0
			&& Sys_Flag.bit.STOP_PWM_Flag_Id == 0 && Sys_Flag.bit.STOP_PWM_Flag_Driv == 0)
		{
			PWMA_Update();
			OpenLoop();
